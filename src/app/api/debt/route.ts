...
import { DebtService, ConflictError, NotFoundError } from '@/services/debt-service';
    ...
    try {
        ...
        if (!parsed.success) {
            return NextResponse.json(
                { error: 'Dados não aceitos. Verifique o formato informado em cada campo.', details: parsed.error.flatten() },
                { status: 400 }
            );
        }
        const service = new DebtService(new DebtRepository());
