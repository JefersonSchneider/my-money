...
export class BillingCycleService {
    ...
    async createBillingCycle(data) {
        ...
        if (existing)
            throw new ConflictError("Billing cycle already exists for this month/year");
        // validar mês
    ...
    async getBillingCycleByMonthYear(month, year) {
        const billingCycle = await this.repository.getByMonthYear(month, year);
        if (!billingCycle)
            throw new NotFoundError("Billing cycle not found");
        return billingCycle;
    }
    async listBillingCyclesByYear(year) {
        // validar ano
    ...
    async getBillingCycleSummary(id) {
        const billingCycle = await this.repository.getById(id);
        if (!billingCycle)
            throw new NotFoundError("Billing cycle not found");
        ...
        const totalPendingDebts = billingCycle.debts?.filter(d => d.status === "PENDENTE").reduce((sum, d) => sum + d.value, 0) || 0;
        const totalPaidDebts = billingCycle.debts?.filter(d => d.status === "PAGO").reduce((sum, d) => sum + d.value, 0) || 0;
        return {
    ...
    async countBillingCycles() {
        return this.repository.count();
    }
    async getBillingCyclesPaginated(page, pageSize = 10) {
        // validar página
        if (page < 1)
